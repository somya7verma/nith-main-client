...
const AddNews = ({ isOpen, onClose, onAdd }: AddNewsProps) => {
  ...
  const handleSubmit = () => {
    if (formData.title && formData.description) {
      onAdd(formData);
      setFormData({ title: '', description: '', date: '', file: '' });
      onClose();
    }
  };
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-lg w-full max-w-2xl max-h-[90vh] overflow-y-auto">
        <div className="sticky top-0 bg-white border-b px-6 py-4 flex items-center justify-between">
          <h2 className="text-xl font-semibold text-gray-900">Add News</h2>
          <button
            onClick={onClose}
        ...
        <div className="p-6">
          <p className="text-gray-600 text-sm mb-6">
            Add a new News. Fill in the details below and click save when
            you&apos;re done
          </p>
